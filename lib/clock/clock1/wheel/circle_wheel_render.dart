...
class RenderCircleListViewport extends RenderBox
  ...
  set scaleOffset(Offset? value) {
    if (value == _scaleOffset) return;
    _scaleOffset = value;
    markNeedsLayout();
  }
  ...
  void setupParentData(RenderObject child) {
    if (child.parentData is! CircleListParentData) child.parentData = CircleListParentData();
  }
  ...
  double _getUntransformedPaintingCoordinate(double layoutCoordinate) {
    // print('----getPixel: layoutCoordinate: $layoutCoordinate, _scrollMarginExtent: $_scrollMarginExtent, offset.pixels: ${offset.pixels}');
  ...
  void performLayout() {
    ...
    final double firstVisibleOffset = offset.pixels + _itemExtent / 2 - visibleSize / 2;
    final double lastVisibleOffset = firstVisibleOffset + visibleSize;
    // print('------firstVisibleOffset: $firstVisibleOffset, lastVisibleOffset: $lastVisibleOffset');
    ...
    // print('------targetFirstIndex: $targetFirstIndex, targetLastIndex: $targetLastIndex');
    ...
    // print('after validates------targetFirstIndex: $targetFirstIndex, targetLastIndex: $targetLastIndex');
  ...
  void paint(PaintingContext context, Offset offset) {
    if (childCount > 0) {
      if (_clipToSize && _shouldClipAtCurrentOffset()) {
        // print('------clipToSize: offset: $offset, size: $size');
        ...
        // print('------_paintVisibleChildren direct');
  ...
  void _paintVisibleChildren(PaintingContext context, Offset offset) {
    // print('------_paintVisibleChildren: offset: $offset, size: $size');
    ...
    while (childParentData != null) {
      testSize++;
      // print('------_paintVisibleChildren: childToPaint: $childToPaint, childParentData.offset: ${childParentData.offset}, index: ${childParentData.index}');
      // print('------_paintVisibleChildren: childToPaint: $childToPaint, childParentData.offset: ${childParentData.offset}');
      _paintTransformedChild(childToPaint, context, offset, childParentData.offset, childParentData.index);
      if (childToPaint != null) childToPaint = childAfter(childToPaint);
      childParentData = childToPaint?.parentData as CircleListParentData?;
    }
    // print('-----------_paintVisibleChildren testSize:$testSize');
  }

  double _testMin = double.infinity;
  double _testMax = double.negativeInfinity;
  ...
  void _paintTransformedChild(
    ...
    // print(
    //     'index:$index----_paintTransformedChild: offset:$offset, layoutOffset:$layoutOffset, untransformedPaintingCoordinates:$untransformedPaintingCoordinates, _scrollMarginExtent: $_scrollMarginExtent');

    // [-_itemExtent, _mainAxisSize]
    final mainCordinate =
        axis == Axis.horizontal ? untransformedPaintingCoordinates.dx : untransformedPaintingCoordinates.dy;

    // print('-----index:$index, mainCordinate: $mainCordinate, _mainAxisSize: $_mainAxisSize, _itemExtent: $_itemExtent');
    // _testMin = math.min(_testMin, mainCordinate);
    // _testMax = math.max(_testMax, mainCordinate);
    // print('-----_testMin: $_testMin, _testMax: $_testMax, mainAxis: $_mainAxisSize, _itemExtent: $_itemExtent');

    final fractional = ((_mainAxisSize / 2) - (mainCordinate + _itemExtent / 2.0)) / (_mainAxisSize / 2);

    // print('index:$index--fractional:$fractional----_paintTransformedChild: mainCordinate: $mainCordinate, _mainAxisSize: $_mainAxisSize, _itemExtent: $_itemExtent');
    ...
    if (axis == Axis.horizontal) {
      ...
      // angle = lerpDouble(0, 2 * math.pi, fractional);
    }

    Matrix4 circleTransform = Matrix4.identity();
    circleTransform.translate(radius * math.cos(angle!) - radius, radius * math.sin(angle) - radius);
    ...
    circleTransform.translate(offsetToCenter.dx, offsetToCenter.dy);

    // print('----index:$index, angle:$angle, fractional:$fractional, scale:$scale');
    // 一个矩形width、height,最右边有一个radius的圆
    // 整体缩放scale
    // 矩形右边间距width * (1-scale)
    // 圆心x方向偏移width * (1-scale) + radius * scale - radius
    // 圆心y方向偏移height * (1-scale)/ 2 + radius * scale - radius

    if (_scaleOffset != null) {
      final scale = lerpDouble(1, 0.7, fractional.abs());
      final fitOffset = Offset(_scaleOffset!.dx * (1 - scale!), _scaleOffset!.dy * (1 - scale));
      if (angle < 0) {
        circleTransform.translate(fitOffset.dx, -fitOffset.dy);
      } else {
        circleTransform.translate(fitOffset.dx, fitOffset.dy);
      }
      circleTransform.scale(scale, scale, 1.0);
    }

    _paintChild(context, offset, child, circleTransform);
  }

...
///    the [CircleFixedExtentScrollController].
class CircleFixedExtentScrollController extends ScrollController {
  /// Creates a scroll controller for scrollables whose items have the same size.
  ///
  /// [initialItem] defaults to 0 and must not be null.
  CircleFixedExtentScrollController({
    this.initialItem = 0,
  });
  ...
  ///
  /// There are circumstances that this [CircleFixedExtentScrollController] can't know
  /// the current item. Reading [selectedItem] will throw an [AssertionError] in
  /// the following cases:
  ///
  /// 1. No scroll view is currently using this [CircleFixedExtentScrollController].
  /// 2. More than one scroll views using the same [CircleFixedExtentScrollController].
  ///
...
/// views such as [CircleListScrollView]s with a [CircleFixedExtentScrollController] and
...
class _FixedExtentScrollPosition extends ScrollPositionWithSingleContext implements FixedExtentMetrics {
  ...
  //   return super.applyContentDimensions(double.negativeInfinity, double.infinity);
  // }
...
/// size so it can pass it on ultimately to the [CircleFixedExtentScrollController].
...
/// that item.
///
/// Must be used with a scrollable that uses a [CircleFixedExtentScrollController].
///
...
class CircleListScrollView extends StatefulWidget {
  ...
  CircleListScrollView({
    ...
    this.radius = 100,
        ...
        childDelegate = CircleListChildLoopingListDelegate(children: children);
  ...
  /// Typically a [CircleFixedExtentScrollController] used to control the current item.
  ///
  /// A [CircleFixedExtentScrollController] can be used to read the currently
  /// selected/centered child item and can be used to change the current item.
  ///
  /// If none is provided, a new [CircleFixedExtentScrollController] is implicitly
  /// created.
  ///
  /// If a [ScrollController] is used instead of [CircleFixedExtentScrollController],
...
class _CircleListScrollViewState extends State<CircleListScrollView> {
  ...
  void initState() {
    super.initState();
    scrollController = widget.controller ?? CircleFixedExtentScrollController();
    if (widget.controller is CircleFixedExtentScrollController) {
      final CircleFixedExtentScrollController controller = widget.controller as CircleFixedExtentScrollController;
      ...
      child: _FixedExtentScrollable(
        ...
        viewportBuilder: (BuildContext context, ViewportOffset offset) {
          ...
          return CircleListViewport(
            ...
            itemExtent: widget.itemExtent,
            scaleOffset: widget.scaleOffset,
...
class CircleListElement extends RenderObjectElement implements CircleListChildManager {
  ...
  Element? updateChild(Element? child, Widget? newWidget, dynamic newSlot) {
    ...
    if (newParentData != null) {
      // debugPrintStack(label: 'updateChild newSlot: $newSlot');
      // print('----------updateChild: newSlot: $newSlot');
...
class CircleListViewport extends RenderObjectWidget {
  ...
  void updateRenderObject(BuildContext context, RenderCircleListViewport renderObject) {
    print('----------updateRenderObject: offset: $offset');
    renderObject
      ..axis = axis
      ..radius = radius
      ..offset = offset
